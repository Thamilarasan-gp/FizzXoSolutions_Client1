...
const AddPathipagamEventForm = () => {
  ...
  const handleSubmit = async (method) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage(method === "PUT" ? "Event updated successfully!" : "Event added successfully!");
        fetchEvents();
        resetForm();
      } else {
        setMessage(result.message || "Error occurred.");
    ...
    <div className="adbook-container" style={{ cursor: loading ? "wait" : "default" }}>
      <div className="ad-bk-fm">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="book-form">
          ...
          <input type="url" name="youtubeLink" value={formData.youtubeLink} onChange={handleChange} placeholder="YouTube Link" />
          <input type="file" name="photo" onChange={handleChange} />
          ...
          <div className="adbk-button-group">
            <button type="button" onClick={() => handleSubmit("POST")}>Add</button>
            <button type="button" onClick={() => handleSubmit("PUT")}>Update</button>
            {selectedEvent && <button type="button" onClick={() => handleDelete(selectedEvent._id)}>Delete</button>}
      ...
      <div className="ad-bk-ds-cont">
        <ul className="book-list">
          {filteredEvents.map((event) => (
            <li
              key={event._id}
              className={`book-item ${selectedEvent?._id === event._id ? "selected" : ""}`}
            ...
            >
              ...
              {event.youtubeLink && (
                <p>
                  <a href={event.youtubeLink} target="_blank" rel="noopener noreferrer">Watch on YouTube</a>
                </p>
              )}
              {event.photoUrl && (
